...
import { Upload, ArrowLeft, LayoutGrid, Share2, GitBranch } from 'lucide-react'
...
interface WelcomeScreenProps {
  onLoadedExcel: (file: ParsedFile, feature: ExcelFeature) => void
  onLoadedPhylo: () => void
}

const LARGE_ROW_THRESHOLD = 5000
const LARGE_COL_THRESHOLD = 50
const PHYLO_LARGE_THRESHOLD = 200
const PHYLO_MAX_SEQS = 1000
...
const FEATURES: {
  ...
  },
  {
    ...
    title: 'PHYLO',
    subtitle: '进化树',
    description: '上传 FASTA 序列，自动检测对齐状态，用 Neighbor-Joining 算法构建系统发育树。',
    tags: ['FASTA', 'NJ-TREE', 'AUTO-ALIGN'],
    icon: GitBranch,
  },
]

const DOT_GRID_CLASS =
  'absolute inset-0 pointer-events-none [background-image:radial-gradient(rgb(var(--ink-faint)/0.35)_1px,transparent_1px)] [background-size:24px_24px] opacity-30'

export function WelcomeScreen({ onLoadedExcel, onLoadedPhylo }: WelcomeScreenProps) {
  ...
  const [pendingLargePhylo, setPendingLargePhylo] = useState<{
    count: number
  } | null>(null)
  const showToast = useUiStore((s) => s.showToast)
  const setSequences = usePhyloStore((s) => s.setSequences)

  const activeFeature = phase !== 'select' ? (phase as Feature) : null

  const proceedExcel = (file: ParsedFile, feature: ExcelFeature) => {
  ...
  const handleExcelUpload = async () => {
    if (!activeFeature || activeFeature === 'phylo') return
    setLoading(true)
    try {
      const file = await loadExcelFromUpload()
      if (file) proceedExcel(file, activeFeature)
    } catch (e) {
  ...
  const proceedPhylo = (
    fileName: string,
    sequences: ReturnType<typeof parseFasta>,
  ) => {
    setSequences(sequences, fileName)
    onLoadedPhylo()
  }
  ...
  const handleFastaUpload = async () => {
    setLoading(true)
    try {
      const loaded = await loadFastaFromUpload()
      if (!loaded) return

      const sequences = parseFasta(loaded.content)
      const validation = validateSequences(sequences)
      if (!validation.ok) {
        const messages: Record<string, string> = {
          'empty': 'FASTA 文件里没找到任何序列',
          'too-few': '至少需要 3 条序列才能建树',
          'duplicate-names': `序列名重复：${validation.duplicateNames?.slice(0, 3).join('、')}`,
        }
        showToast('error', messages[validation.issue!] ?? '文件无效')
        return
      }

      if (sequences.length > PHYLO_MAX_SEQS) {
        showToast(
          'error',
          `序列数 ${sequences.length} 超过上限 ${PHYLO_MAX_SEQS}，请精简后再试`,
        )
        return
      }

      if (sequences.length > PHYLO_LARGE_THRESHOLD) {
        setPendingLargePhylo({ count: sequences.length })
        // stash for confirm dialog flow
        pendingPhyloRef.fileName = loaded.fileName
        pendingPhyloRef.sequences = sequences
        return
      }

      proceedPhylo(loaded.fileName, sequences)
    } catch (e) {
      const msg = e instanceof Error ? e.message : '解析失败，请检查文件'
      showToast('error', `FASTA 解析失败：${msg}`)
    } finally {
      setLoading(false)
    }
  }

  const handleUpload = activeFeature === 'phylo' ? handleFastaUpload : handleExcelUpload

  const featureExamples = activeFeature && activeFeature !== 'phylo'
  ...
  if (phase === 'select') {
    return (
      <div className="min-h-screen bg-cream text-ink relative">
        ...
        <div className="px-10 py-16 max-w-4xl mx-auto relative">
          <div className="mb-14">
          ...
          <div className="grid grid-cols-3 gap-4">
            {FEATURES.map((f, idx) => {
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between mb-7 pb-3 border-b border-line group-hover:border-terracotta/30 transition-colors">
                  ...
                  <h2 className="font-mono text-xl font-medium text-ink mb-1 tracking-tight">
                    {f.title}
                  </h2>
                  ...
                  <p className="text-sm text-ink-muted leading-relaxed mb-6 min-h-[60px]">
                    {f.description}
                  </p>
                  ...
                  <div className="flex flex-wrap gap-1.5 mb-5">
                    {f.tags.map((t) => (
                      <span
                        key={t}
                        className="font-mono text-[10px] px-1.5 py-0.5 border border-line text-ink-subtle group-hover:text-ink-muted transition-colors"
                      >
                        {t}
                      </span>
                    ))}
                  ...
                  <div className="flex items-center justify-between pt-3 border-t border-line group-hover:border-terracotta/30 transition-colors">
          ...
          <div className="mt-16 font-mono text-[10px] uppercase tracking-widest text-ink-faint flex items-center gap-2">
  ...
  const isPhylo = activeFeature === 'phylo'
  ...
  return (
    <div className="min-h-screen bg-cream text-ink relative">
        ...
        <div className="border border-line bg-cream-50/60 p-8 mb-8">
          ...
          >
            ...
            <span className="uppercase tracking-wider">
              {loading ? 'PARSING…' : isPhylo ? 'UPLOAD FASTA' : 'UPLOAD EXCEL'}
            </span>
          </button>
          {isPhylo && (
            <p className="mt-3 font-mono text-[10px] uppercase tracking-wider text-ink-faint">
              [ .FASTA / .FA / .FNA / .FAA / .TXT · 3 – {PHYLO_MAX_SEQS} SEQUENCES ]
            </p>
          )}
        </div>
            ...
            <div className="grid grid-cols-1 gap-2">
              {featureExamples.map((ds, idx) => (
                <button
                  key={ds.id}
                  onClick={() =>
                    proceedExcel(ds.data, activeFeature as ExcelFeature)
                  }
                  className={cn(
      ...
      <ConfirmDialog
        ...
        onConfirm={() => {
          if (pendingLarge) {
            onLoadedExcel(pendingLarge.file, pendingLarge.feature)
      ...
      <ConfirmDialog
        open={pendingLargePhylo !== null}
        onOpenChange={(open) => !open && setPendingLargePhylo(null)}
        title="序列较多"
        description={
          pendingLargePhylo && (
            <>
              FASTA 里有 <strong>{pendingLargePhylo.count}</strong> 条序列，
              建树（O(n³)）可能需要数秒到十几秒。是否继续？
            </>
          )
        }
        confirmLabel="继续建树"
        onConfirm={() => {
          if (pendingLargePhylo && pendingPhyloRef.sequences && pendingPhyloRef.fileName) {
            proceedPhylo(pendingPhyloRef.fileName, pendingPhyloRef.sequences)
            pendingPhyloRef.sequences = null
            pendingPhyloRef.fileName = null
            setPendingLargePhylo(null)
          }
        }}
      />
    </div>
...
// Module-level stash for confirm dialog flow (avoids re-passing through state)
const pendingPhyloRef: {
  sequences: ReturnType<typeof parseFasta> | null
  fileName: string | null
} = { sequences: null, fileName: null }
